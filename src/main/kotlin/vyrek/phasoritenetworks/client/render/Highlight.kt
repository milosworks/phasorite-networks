package vyrek.phasoritenetworks.client.render
...
/**
 * Kotlin wrapper for owo's Color class to use destructuring
 */
data class KColor(val alpha: Float, val red: Float, val green: Float, val blue: Float) {
	companion object {
		fun of(color: Color): KColor {
			return KColor(color.alpha, color.red, color.green, color.blue)
		}

		fun toRgb(color: Int): KColor {
			val raw = Color.ofArgb(color)

			return KColor(1f, raw.red, raw.green, raw.blue)
		}
	}
}
...
class Highlight :
	...
	companion object {
		...
		val OUTLINE_COLOR = KColor.of(Color.RED)
	...
	fun add(pos: BlockPos, time: Long, color: Int?) {
		blocksToRender.add(BlockRenderData(pos, time, color?.let { KColor.toRgb(it) } ?: OUTLINE_COLOR))
	}

	/**
	 * Called on every tick on RenderLevelStageEvent after particles
	 * @see vyrek.phasoritenetworks.init.PNGameEvents.renderLevelStage
	 */
